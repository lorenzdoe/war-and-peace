...
def read_wordlists(war_file: str, peace_file: str)-> Maybe:
    ...
    """
    war_keywords = read_keywords('in/war.in')
    peace_keywords = read_keywords('in/peace.in')
...
def split_into_chapters(book: List[str], keyword: str) -> List[List[str]]:
    """
    ...
    return list(only_words_chapters)[1:]
...
def calc_score(indices: Tuple[int], nr: int, chap_len: int) -> Dict[int, float]:
    """
    shuffle step from map reduce, returns a dict with key is chapter nr. and value is score for that chapter
    """
    ...
    return {
        nr: (space_weight * space_score + density_weight * density_score ) * 100
    }


if __name__ == '__main__':

    wordlist: dict = read_wordlists('in/war.in', 'in/peace.in').value
    war_and_peace: List[str] = read_and_split('in/war_and_peace.in').value
    chapters: List[List[str]] = split_into_chapters(book=war_and_peace,keyword='CHAPTER')
    transformed_chapters = [transform_chapter(wordlist)(chapter) for chapter in chapters]
    mapped_chapters = [map_chapter_WoP(wordlist, chapter, i+1) for i, chapter in enumerate(chapters)]
    
    for map_chap in mapped_chapters:
        war_score = calc_score(map_chap[WarOrPeace.WAR], map_chap['nr'], map_chap['len'])[map_chap['nr']]
        peace_score = calc_score(map_chap[WarOrPeace.PEACE], map_chap['nr'], map_chap['len'])[map_chap['nr']]
        res = 'peace-related' if war_score < peace_score else 'war-related'
        print(f"Chapter {map_chap['nr']}: {res}")
